...
async def lifespan(app: FastAPI):
    ...
    logger.info(f"Model loaded: {settings.model_name}. Context length: {context_len}")
    yield


app = FastAPI(title="VILA/NVILA Service", lifespan=lifespan)
...
async def chat_completions(request: ChatCompletionRequest):
    ...
    if request.model != settings.model_name:
        err_msg = (
            f"The endpoint is configured to use the model {settings.model_name}, "
            f"but the request model is {request.model}"
        )
        logger.error(err_msg)
        return JSONResponse(status_code=400, content=ErrorResponse(error=err_msg).dict())
    try:
        result = await handle_request(request, model, tokenizer, image_processor, settings.conv_mode)
...
if __name__ == "__main__":
    uvicorn.run(app, host=settings.host, port=settings.port, workers=settings.num_workers)
